...
exports.submitReplay = exports.getLeaderboard = exports.submitToTournament = exports.registerReplay = exports.uploadReplay = void 0;
 ...
 */
async function uploadReplay(replayPath) {
    console.log(`📤 Uploading replay to IPFS...`);
    if (!fs_1.default.existsSync(replayPath)) {
        throw new Error(`Replay file not found: ${replayPath}`);
    }
    try {
        const FormData = require('form-data');
        const formData = new FormData();
        formData.append('file', fs_1.default.createReadStream(replayPath));
 ...
 */
async function registerReplay(missionId, replayUrl, score) {
    console.log(`📝 Registering replay on blockchain...`);
    try {
        const response = await axios_1.default.post(`${REPLAY_API_URL}/register`, {
            missionId,
            replayUrl,
            score
        });
...
exports.registerReplay = registerReplay;
/**
 * Submit replay to tournament
 */
async function submitToTournament(missionId, replayUrl, score) {
    console.log(`🏆 Submitting to tournament...`);
    try {
        const response = await axios_1.default.post(`${TOURNAMENT_API_URL}/entries/submit`, {
            missionId,
            replayUrl,
            score
        });
...
exports.submitToTournament = submitToTournament;
/**
 * Get tournament leaderboard
 */
async function getLeaderboard(missionId) {
 ...
 */
function parseReplayMetadata(replayPath) {
 ...
 * Complete workflow
 */
async function submitReplay(replayPath, missionId, score) {
...
exports.submitReplay = submitReplay;
/**
 * CLI entry point
 */
async function main() {
